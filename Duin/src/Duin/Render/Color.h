...
/**
 * @struct Color
 * @brief RGBA color with multiple constructors.
 * @ingroup Render_Core
 *
 * Stores colors as normalized floats [0,1]. Provides conversion to
 * byte arrays and packed formats for BGFX.
 */
struct Color
{
    float r, g, b, a; ///< Color components [0.0 - 1.0].

    /** @brief Constructs from normalized floats [0,1]. */
    Color(float _r, float _g, float _b, float _a = 1.0f) : r{_r}, g{_g}, b{_b}, a{_a}
    {
    }

    /** @brief Constructs from integer values [0-255]. */
    Color(int _r, int _g, int _b, int _a = 255) : r{_r / 255.0f}, g{_g / 255.0f}, b{_b / 255.0f}, a{_a / 255.0f}
    {
    }

    /** @brief Constructs from hex value (0xRRGGBB or 0xRRGGBBAA). */
    Color(uint32_t hex)
    {
        if (hex > 0xFFFFFFu)
        {
    ...
    {
        auto clamp = [](float x) { return uint8_t(std::clamp(x * 255.0f, 0.0f, 255.0f)); };
