...
std::shared_ptr<DynamicBody> DynamicBody::Create(Transform3D transform, CollisionGeometryVariant geometry,
                                                 PhysicsMaterial material)
{
    return std::make_shared<DynamicBody>(transform, geometry, material);
}

DynamicBody::DynamicBody(Transform3D transform, CollisionShape collisionShape) : collisionShape(collisionShape)
{
