...
class Iter
{
  public:
    ...
    /**
     * @brief Get the user parameter passed to System::Run(deltaTime, param).
     *
     * This is the `param` argument forwarded to ecs_run; it is null when the
     * system runs from the pipeline (World::Progress) or when no param was given.
     * @return The raw parameter pointer, or nullptr if none was passed.
     */
    void *Param() const
    {
        return iter_->param;
    ...
    /**
     * @brief Typed accessor for the user parameter passed to System::Run.
     * @tparam T The type the parameter was passed as.
     * @return The parameter reinterpreted as T*, or nullptr if none was passed.
     *
     * The caller is responsible for ensuring T matches the type of the object
     * whose address was passed to Run(); no type checking is performed.
     */
    template <typename T>
    T *Param() const
    {
