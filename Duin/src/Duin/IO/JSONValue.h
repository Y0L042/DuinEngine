...
class JSONValue
{
  public:
    ...
    JSONValue &AddMember(const std::string &key, T val, bool allowDuplicates = false) // Add Key:Value member to object
    {
        ...
        {
            v.SetString(val.c_str(), static_cast<rapidjson::SizeType>(val.size()), jdoc_->GetAllocator());
        }
        else if constexpr (std::is_same_v<T, char>)
        {
            char buf[2] = {val, '\0'};
            v.SetString(buf, static_cast<rapidjson::SizeType>(sizeof(char)), jdoc_->GetAllocator());
        }
        else
        {
            v.Set(val);
        ...
        jvalue_->AddMember(std::move(name), std::move(v), jdoc_->GetAllocator());
