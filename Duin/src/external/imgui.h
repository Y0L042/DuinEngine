...
// - (note: before v1.91.4 (2024/10/08) the default type for ImTextureID was void*. Use intermediary intptr_t cast and
// read FAQ if you have casting warnings)
#ifndef ImTextureID
typedef ImU64 ImTextureID; // Default: store a pointer or an integer fitting in a pointer (most renderer backends are ok
                           // with that)
#endif
...
IMGUI_API void ImageWithBg(ImTextureID user_texture_id, const ImVec2 &image_size, const ImVec2 &uv0 = ImVec2(0, 0),
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_NavLeftJumpsBackHere = 1 << 17, // (WIP) Nav: left direction may move to this TreeNode() from any
...
struct ImFontConfig
{
    ...
    float RasterizerDensity;  // 1.0f     // DPI scale for rasterization, not altering other font metrics: make it easy
                              // to swap between e.g. a 100% and a 400% fonts for a zooming display, or handle Retina
                              // screen. IMPORTANT: If you change this it is expected that you increase/decrease font
...
struct ImFontAtlas
{
    ...
    IMGUI_API void ClearFonts();     // Clear input+output font data (same as ClearInputData() + glyphs storage, UV
                                     // coordinates).
...
struct ImFont
{
    ...
    short SourcesCount; // 2     // in  // Number of ImFontConfig involved in creating this font. Usually 1, or >1 when
    ...
    float Scale; // 4     // in  // Base font scale (1.0f), multiplied by the per-window font scale which you can adjust
                 // with SetWindowFontScale()
    float Ascent, Descent;   // 4+4   // out // Ascent: distance from top to bottom of e.g. 'A' [0..FontSize] (unscaled)
    int MetricsTotalSurface; // 4     // out // Total surface in pixels to get an idea of the font rasterization/texture
...
IMGUI_API void Image(ImTextureID user_texture_id, const ImVec2 &image_size, const ImVec2 &uv0, const ImVec2 &uv1,
