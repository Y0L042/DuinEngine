...
static DebugShapeVertex s_cubeVertices[8] = {
    {-1.0f, 1.0f, 1.0f, {0, 0, 0, 0}},   {1.0f, 1.0f, 1.0f, {0, 0, 0, 0}},   {-1.0f, -1.0f, 1.0f, {0, 0, 0, 0}},
    {1.0f, -1.0f, 1.0f, {0, 0, 0, 0}},   {-1.0f, 1.0f, -1.0f, {0, 0, 0, 0}}, {1.0f, 1.0f, -1.0f, {0, 0, 0, 0}},
        ...
        struct Gen
        {
                  ...
                  m_normalStride(_normalStride)
            {
                ...
                static const bx::Vec3 vv[] = {
                    {-ll, 0.0f, -ss}, {ll, 0.0f, -ss}, {ll, 0.0f, ss},  {-ll, 0.0f, ss},

                    {-ss, ll, 0.0f},  {ss, ll, 0.0f},  {ss, -ll, 0.0f}, {-ss, -ll, 0.0f},

                    {0.0f, -ss, ll},  {0.0f, ss, ll},  {0.0f, ss, -ll}, {0.0f, -ss, -ll},
                };
...
struct GeometryT
{
                          ...
                          bool _index32)
    {
        ...
        if (isValid(handle))
        {
            ...
            geometry.m_topologyNumIndices[1] = bgfx::topologyConvert(bgfx::TopologyConvert::TriListToLineList, NULL, 0,
                                                                     _indices, _numIndices, _index32);
...
struct DebugDrawShared
{
    ...
    GeometryHandle createGeometry(uint32_t _numVertices, const DdVertex *_vertices, uint32_t _numIndices,
                                  const void *_indices, bool _index32)
    {
...
struct DebugDrawEncoderImpl
{
    ...
    void draw(GeometryHandle _handle)
    {
        ...
        if (wireframe)
        {
            m_encoder->setIndexBuffer(geometry.m_ibh, geometry.m_topologyNumIndices[0],
              ...
              const uint16_t *_indices)
    {
        ...
        if (_numVertices == bgfx::getAvailTransientVertexBuffer(_numVertices, DebugMeshVertex::ms_layout))
        {
            ...
            if (0 < _numIndices)
            {
                ...
                {
                    numIndices = bgfx::topologyConvert(bgfx::TopologyConvert::TriListToLineList, NULL, 0, _indices,
                                                       _numIndices, false);

                    bgfx::allocTransientIndexBuffer(&tib, numIndices);
                    bgfx::topologyConvert(bgfx::TopologyConvert::TriListToLineList, tib.data,
                                          numIndices * sizeof(uint16_t), _indices, _numIndices, false);
                }
                else
                {
                    bgfx::allocTransientIndexBuffer(&tib, numIndices);
    ...
    void drawFrustum(const float *_viewProj)
    {
        ...
        const bx::Vec3 points[8] = {
            intersectPlanes(planes[0], planes[2], planes[4]), intersectPlanes(planes[0], planes[3], planes[4]),
            intersectPlanes(planes[0], planes[3], planes[5]), intersectPlanes(planes[0], planes[2], planes[5]),
            intersectPlanes(planes[1], planes[2], planes[4]), intersectPlanes(planes[1], planes[3], planes[4]),
            intersectPlanes(planes[1], planes[3], planes[5]), intersectPlanes(planes[1], planes[2], planes[5]),
        };
    ...
    void drawCircle(Axis::Enum _axis, float _x, float _y, float _z, float _radius, float _weight)
    {
        ...
        for (uint32_t ii = 1; ii < num; ++ii)
        {
            ...
            pos = getPoint(_axis, bx::lerp(xy0[0], xy1[0], _weight) * _radius,
...
GeometryHandle ddCreateGeometry(uint32_t _numVertices, const DdVertex *_vertices, uint32_t _numIndices,
                                const void *_indices, bool _index32)
{
...
void DebugDrawEncoder::drawQuad(bgfx::TextureHandle _handle, const bx::Vec3 &_normal, const bx::Vec3 &_center,
                                float _size)
{
    DEBUG_DRAW_ENCODER(drawQuad(_handle, _normal, _center, _size));
}
